...
def analyse_falling_drop(settings, video_file):
    # Steps

    # preproccessing, background subtraction etc

    # get full outline and end positions for whole
    # video

    # find where the droplet enters and hits the ground
    # to get starting and ending frame

    # Open file based on frames, and get full thresholded frame
    if settings['CROP']:
        pass
    ...
    # 2 drop falling
    # 3 drop impacting
    # within drop impacting there are a few other things that need to
    # be accounted for, however that should be within it's own frame

    pass


def threshold_video(settings, video_file):
    """
    returns: thresholded video, with settings in settings

    optionally can subtract background, or remove small holes
    in the image, or display video, thresholded video and background
    which is subtracted
    """


def analyse_droplet_falling_single_frame(settings, threshold_frame):
    """ Returns an array of data if the droplet is fully in the frame
    or False otherwise """

    # data this gives out
    # exremum positions, com, volume estimations (versions ?), surface area
    # estimation
    pass


def calculate_velocities_and_weber_numbers(settings, frame_data):
    pass


def graph_falling_summary(setting, frame_data):
    pass
